      ...
      <span className="block h-auto rounded-lg">
        ...
        {reference.contributors.map((contributor: any) => (
          <div key={contributor._id}>
            {contributor.contributorFirstName} {contributor.contributorMiddleI}{" "}
            {contributor.contributorLastName}
            <br></br>
          </div>
        ))}
      </span>
...
function ReferenceActions({ onEdit, onDelete, onExport }: any) {
  return (
    <div>
      <Button color="green" onClick={onEdit}>
        Edit
      </Button>
      <Button color="red" onClick={onDelete}>
        Delete
      </Button>
      <Button color="orange" onClick={onExport}>
        Export
      </Button>
    </div>
  );
...
const ViewReference = () => {
  ...
  async function exportCitation() {
    ...
    // Navigate to the new page for selecting CSL and Locale options
    router.push(
      `/${reference._id}/selectCitationStyle?referenceId=${referenceId}`
    );
  }
  ...
  const handleEdit = () => {
    router.push(
      `/${reference._id}/edit?id=${encodeURIComponent(reference._id)}`
    );
  };
