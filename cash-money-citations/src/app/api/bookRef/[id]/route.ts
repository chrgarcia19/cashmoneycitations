...
import BookRef from "@/models/reference-models/BookRef";
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const reference = await BookRef.findById(id);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const deletedReference = await BookRef.deleteOne({ _id: id });
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const reference = await BookRef.findByIdAndUpdate(id, req, {
      new: true,
      runValidators: true,})
