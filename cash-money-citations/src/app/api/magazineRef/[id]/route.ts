...
import MagazineRef from "@/models/reference-models/MagazineRef";
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const reference = await MagazineRef.findById(id);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const deletedReference = await MagazineRef.deleteOne({ _id: id });
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const reference = await MagazineRef.findByIdAndUpdate(id, req, {
      new: true,
      runValidators: true,})
