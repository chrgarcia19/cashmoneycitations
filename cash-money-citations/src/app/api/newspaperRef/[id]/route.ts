...
import NewspaperRef from "@/models/reference-models/NewspaperRef";
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const reference = await NewspaperRef.findById(id);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const deletedReference = await NewspaperRef.deleteOne({ _id: id });
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const reference = await NewspaperRef.findByIdAndUpdate(id, req, {
      new: true,
      runValidators: true,})
